// ============================================
// TERMOS URC - GERA O HTML DO DOCUMENTO (para virar PDF no Gotenberg)
// Dinâmico: usa o HTML do termo enviado pelo sistema (documento.html).
// Sem HTML recebido, mantém o Termo de Consentimento (LGPD) original.
// ============================================
...
// Documento (Termos URC) — quando ausente, cai no LGPD
const tipoDocumento = dadosNode.TipoDocumento || "termo-lgpd";
const nomeDocumento = dadosNode.NomeDocumento || "Termo de Consentimento (LGPD)";
const documentoId = dadosNode.DocumentoId || "";
let htmlDocumento = String(dadosNode.HtmlDocumento || "").trim();

// O fragmento vindo do sistema já traz o título em <h3>; o layout abaixo
// imprime o título no cabeçalho, então removemos a duplicidade.
...
const conteudoHash = `${nomeCompleto}|${cpfFormatado}|${nomeDocumento}|${dataEnvio}|${Date.now()}`;
...
//    - Termos URC: HTML recebido do sistema
//    - Sem HTML: texto padrão do Termo LGPD
// =============================================
const corpoLGPD = `
...
<head>
  ...
  <style>
    /* Estilos das classes usadas pelos termos gerados no sistema */
  ...
  <div style="max-width:700px;margin:0 auto;background:#ffffff;overflow:hidden;">
    ...
    <div style="padding:15px 40px 8px 40px;line-height:1.6;color:#333333;font-size:13.5px;">
      ...
      <div style="text-align:center;padding:18px 0 12px 0;">
        <p style="margin:0 0 6px 0;font-size:12.5px;font-weight:700;color:#333333;font-family:'Courier New',Courier,monospace;text-align:center;">Hash SHA-256:</p>
    ...
    <div style="padding:15px 40px;text-align:right;border-top:1px solid #e0e0e0;">
      <p style="margin:0;font-size:10.5px;color:#999999;font-style:italic;text-align:right;">Documento gerado em ${dataHoraGeracao}</p>
    </div>
